...
if (old === null) {
 return sendEmbed(message, `ไม่มีการตอบกลับ / แท็กที่กำหนดเองดังกล่าว!`, "RED")
}
...
if (old === null) {
 ...
 return sendFollowUp(interaction, `ไม่มีการตอบกลับ / แท็กที่กำหนดเองดังกล่าว!`, "RED")
}

await db.delete(`reply_${interaction.guild.id}_${cmd}`)
return sendFollowUp(interaction, `ลบคําที่ตอบกลับ \`${cmd}\` เเละคําตอบของการตอบกลับเเล้ว \`${old}\``, "GREEN")
  }
};
